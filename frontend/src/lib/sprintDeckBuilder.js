    ...
    if (items.length) {
      const runs = items
        .map((t, i) => {
          ...
          const para = parseRuns(text).map((r) => ({ text: r.text, options: { bold: r.bold, color: r.bold ? P.TXT_BOLD : P.TXT_NORMAL } }));
        ...
        .flat();
      s2.addText(runs, { x: x + G.ACC_ZONE, y: y + G.PAD_T + G.TITLE_H, w: G.COL_W - G.ACC_ZONE - G.PAD_R, h: h - G.PAD_T - G.TITLE_H - G.PAD_B + 0.05, fontFace: "Calibri", fontSize: fs2, color: P.TXT_NORMAL, valign: "top", margin: 0, lineSpacingMultiple: 1.0 });
    }
    return h;
  }
